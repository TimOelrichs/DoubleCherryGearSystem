...
void GearsystemCore::SetGearToGearTarget(GearsystemCore* target) { this->m_pGameGearIOPorts->setLinkedPorts(target->m_pGameGearIOPorts); };
...
bool GearsystemCore::RunStep(u8* pFrameBuffer, s16* pSampleBuffer, int* pSampleCount, bool step, bool stopOnBreakpoints)
{
    bool breakpoint = false;
    bool vblank = false;
       ...
       // while (!vblank)
        {
            ...
            m_pAudio->Tick(clockCycles);
            m_pInput->Tick(clockCycles);
            m_pGameGearIOPorts->UpdateSerial(clockCycles);
            totalClocks += clockCycles;
