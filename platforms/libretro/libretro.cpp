...
static u8 number_of_emulated_gamegears = 1;
static bool horizontal_g2g_slitscreen = true;
static int16_t audio_mix_buf[2];
static u8 g2g_audio_mode = 0;
struct retro_system_av_info* current_av_info = (retro_system_av_info*)malloc(sizeof(*current_av_info));
...
static const struct retro_variable vars[] = {
    ...
    { "gearsystem_gear2gear", "Gear2Gear Emulated Game Gears ; 1|2" },
    { "gearsystem_splitscreen", "Gear2Gear SplitScreen ; horizontal|vertical" },
    { "gearsystem_g2g_audio", "Gear2Gear Audio ; Game Gear1 only|Game Gear2 only|Both mixed" },
    { NULL }
};
...
static void update_multiplayer_geometry()
{
    ...
    if(number_of_emulated_gamegears == 1)
    {
    ...
    else
    {
        ...
        if(horizontal_g2g_slitscreen)
        {
          current_screen_width = runtime_info.screen_width * 2;
          current_screen_height = runtime_info.screen_height;
         }
        else
        {
    ...
    current_av_info->geometry.base_width = current_screen_width;
    current_av_info->geometry.base_height = current_screen_height;
    current_av_info->geometry.aspect_ratio = static_cast<float>(current_screen_width) / current_screen_height;

    environ_cb(RETRO_ENVIRONMENT_SET_GEOMETRY, current_av_info);
...
static void mix_audio_2p(
    int16_t* out,
    const int16_t* a,
    const int16_t* b,
    int samples /* stereo samples count */
)
{
    for (int i = 0; i < samples; i++)
    {
        int mixed = (a[i] + b[i]) >> 1; // 50% + 50%
        out[i] = clamp16(mixed);
    }
}
...
void retro_get_system_av_info(struct retro_system_av_info *info)
{
    ...
    memcpy(current_av_info, info, sizeof(*current_av_info));
...
static void check_variables(void)
{
   ...
   var.key = "gearsystem_gear2gear";
   var.value = NULL;

    if (environ_cb(RETRO_ENVIRONMENT_GET_VARIABLE, &var) && var.value)
    {
           number_of_emulated_gamegears = atoi(var.value);
        update_multiplayer_geometry();
    }

    var.key = "gearsystem_splitscreen";
    var.value = NULL;

    if (environ_cb(RETRO_ENVIRONMENT_GET_VARIABLE, &var) && var.value)
    {
           horizontal_g2g_slitscreen = (strcmp(var.value, "horizontal") == 0);
            update_multiplayer_geometry();
    }

     var.key = "gearsystem_g2g_audio";
    var.value = NULL;

    if (environ_cb(RETRO_ENVIRONMENT_GET_VARIABLE, &var) && var.value)
    {
         if (strcmp(var.value, "Game Gear1 only") == 0)
           g2g_audio_mode = 0;
        else if (strcmp(var.value, "Game Gear2 only") == 0)
           g2g_audio_mode = 1;
        else
            g2g_audio_mode = 2;
...
/*
    ...
    video_cb((uint8_t*)frame_buffers[0], runtime_info.screen_width, runtime_info.screen_height, runtime_info.screen_width * sizeof(u8) * 2);
    ...
    bool vblank2 = false;
    //serial
    //while (!vblank1 && !vblank2)
    while (!(vblank1 && vblank2))
    {
        if(!vblank1)
            vblank1 = cores[0]->RunStep(frame_buffers[0], audio_buf[0], &audio_sample_count[0]);
        if(!vblank2)
            vblank2 = cores[1]->RunStep(frame_buffers[1], audio_buf[1], &audio_sample_count[1]);
    }
    ...
    if(horizontal_g2g_slitscreen)
    {
     static u8 joined_buf[
        GS_RESOLUTION_GG_WIDTH * 2 *
        GS_RESOLUTION_GG_HEIGHT *
        2 /* bytes per pixel */
    ];

    const int src_pitch  = GS_RESOLUTION_GG_WIDTH * 2;
    const int dst_pitch  = GS_RESOLUTION_GG_WIDTH * 2 * 2;

    for (int y = 0; y < GS_RESOLUTION_GG_HEIGHT; y++)
    {
        // left GG
        memcpy(
            joined_buf + y * dst_pitch,
            frame_buffers[0] + y * src_pitch,
            src_pitch
        );

        // right GG
        memcpy(
            joined_buf + y * dst_pitch + src_pitch,
            frame_buffers[1] + y * src_pitch,
            src_pitch
        );
    }
...
if(g2g_audio_mode < 2){
 if (audio_sample_count[g2g_audio_mode] > 0)
        audio_batch_cb(audio_buf[g2g_audio_mode], audio_sample_count[g2g_audio_mode] / 2);
     return;
}

// --- AUDIO MIX ---
int samples0 = audio_sample_count[0];
int samples1 = audio_sample_count[1];

// Sicherheit: kleinste Sample-Anzahl nehmen
int mixed_samples = samples0 < samples1 ? samples0 : samples1;
...
if (mixed_samples > 0)
{
    ...
    // libretro erwartet Frames, nicht Einzel-Samples
    audio_batch_cb(audio_mix_buf, mixed_samples / 2);
}
